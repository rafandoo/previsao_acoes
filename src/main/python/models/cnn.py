...
from keras._tf_keras.keras.layers import Dense, Conv1D, MaxPooling1D, Dropout, Flatten, BatchNormalization
from util.logger import Logger
import numpy as np
from typing import Optional, List, Tuple, override
...
class mCNN(InterfaceModels):
    """
    Classe que implementa o modelo CNN.
    ...
    def __init__(
        ...
        self._id = id
        self._ticker = ticker
        self._model = Sequential(name='CNN')
        ...
        if (len(activation_functions) == ModelEnum.CNN_ACTIVATION_FUNCTIONS_SIZE.value):
    ...
    def fit(self) -> None:
        ...
        try:
            ...
            Logger.notice(
                'Treinamento do modelo {} finalizado com sucesso!', self.name)
            Logger.debug(
                'Resumo do treinamento: Última época interrompida em {}', self._stopped_epoch)
        except Exception as e:
    ...
    def predict(self) -> None:
        ...
        try:
            ...
            self._output = (
                DataProcessor.denormalize(
                    predictions, self._scaler[0], self._scaler[1]),
                DataProcessor.denormalize(
                    self._y_norm_test, self._scaler[0], self._scaler[1])
            )
